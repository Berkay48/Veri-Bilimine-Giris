# -*- coding: utf-8 -*-
"""Ford_Araba_Satış_Tahmini(rf)Son.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1XNTNX_uqc4txO07esd-tIoKdbERRCk_r
...
"""En Yaşlı Ve Genç Arabayı Tespi Etme"""
...
rf_model = RandomForestRegressor(n_estimators=300,max_depth=10,min_samples_split=6,min_samples_leaf=2,max_features='sqrt',bootstrap=True,n_jobs=-1,random_state=42,oob_score=True,verbose=0)
...
plt.plot(y_test.values, label='Gerçek Fiyat', alpha=0.7)
plt.plot(y_pred, label='Tahmin Fiyat (RandomForest)', alpha=0.7)
plt.title('RandomForest - Test Setinde Gerçek vs Tahmin Edilen Araç Fiyatları')
plt.xlabel('Gözlem')
plt.ylabel('Fiyat')
plt.legend()
plt.grid(True)
...
plt.plot(y_train_pred, label='Tahmin Fiyat (RandomForest)', alpha=0.7)
plt.title('RandomForest - Eğitim Setinde Gerçek vs Tahmin Edilen Araç Fiyatları')
plt.xlabel('Gözlem')
plt.ylabel('Fiyat')
plt.legend()
plt.grid(True)
